		...
		if (type == IMGTYPE_COLORALPHA)
		{
			...
			else if( r_qs_postprocess->value )
			{
				for ( i = 0; i < c; i++ )
				{
					...
					scan[i*4] = LERP(scan[i*4], luma, r_qs_postprocess->value);
					scan[i*4 + 1] = LERP(scan[i*4 + 1], luma, r_qs_postprocess->value);
					scan[i*4 + 2] = LERP(scan[i*4 + 2], luma, r_qs_postprocess->value);
				}
			}
