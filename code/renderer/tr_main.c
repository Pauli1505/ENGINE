...
static qboolean R_GetPortalOrientations(const drawSurf_t *drawSurf, int entityNum,
    ...
    for (i = 0; i < tr.refdef.num_entities; i++) {
        ...
        // Calculate the distance from the camera to this portal
        d = DistanceSquared(cameraOrigin, e->e.origin); // Use squared distance for efficiency
        if (d > 64 || d < -64) {
            continue;
        }

        // Check if this is the closest portal
        if (d < closestDistance) {
            closestDistance = d;
            closestPortal = e; // Set the closest portal entity
        }
    }

    if (closestPortal) {
        // Use the closest portal
        e = closestPortal;

        // Get the PVS origin from the closest portal entity
