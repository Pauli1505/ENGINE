...
if [ ! -f Makefile ]; then
	echo "$0 must be run from the sandbox build directory"
	exit 1
fi

if [ "${Q3E_CLIENT_ARCHS}" == "" ]; then
	echo "$0: no sandbox binary architectures were found for target '${TARGET_NAME}'"
	exit 1
fi
...
<dict>
    ...
    <string>org.sandbox.${PRODUCT_NAME}</string>
